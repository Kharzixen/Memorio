...
@AllArgsConstructor
@Slf4j
public class PublicAlbumEventListener {
    private final ObjectMapper objectMapper;
    private final MinioClient minioClient;
    private final AlbumService albumService;

    @KafkaListener(topics = "memorio.publicalbumdb_test.memory_event_outbox", groupId = "media-service-public-album-listener")
    public void memoryEventListener(@Payload(required = false) String message, @Header(KafkaHeaders.RECEIVED_KEY) String key) {
        log.info("Received public memory message: {}", message);
        ...
        try {
            ...
            minioClient.removeObject(RemoveObjectArgs.builder().bucket("public-album-bucket").object(imageId).build());
            return;
        } catch (Exception e) {
